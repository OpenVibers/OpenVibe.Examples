 ...
 * openvibe-sdk/events fills event_id (evt_<ULID>), timestamp, version and trace_id. Publishing the
 * same event_id again is answered with the stored seq (duplicate: true), so retries are safe.
 */
...
const { projectKey, appSource } = require('./subscriber');
...
async function publishAppEvent(config, { name = 'example.ping', subject = { type: 'example', id: '1' }, payload = {}, eventId } = {}, { fetch } = {}) {
    ...
    const eventType = `app.${projectKey(projectId)}.${name}`;
    const events = createEventsClient(client, { source: appSource(config.clientId) });
    const out = await events.publish({
        ...(eventId ? { event_id: eventId } : {}),
        event_type: eventType,
        actor: { type: 'app', id: config.clientId },
        subject,
        payload,
    });
    return { ...out, event_type: eventType };
}
